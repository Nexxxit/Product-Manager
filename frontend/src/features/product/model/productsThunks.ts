...
import {productsApi, USE_API_IN_DEV_FLAG} from "./productsApi.ts";
import type {NewProduct, Product, UpdateProductInput} from "./types.ts";
import type {RootState} from "../../../app/store";

const USE_API_IN_DEV = USE_API_IN_DEV_FLAG;

export const fetchProducts = createAsyncThunk<Product[]>(
    'product/fetchAll',
    async () => {
        if (import.meta.env.DEV && !USE_API_IN_DEV) {
...
export const deleteProduct = createAsyncThunk<number, number>(
    'product/delete',
    async (id) => {
        if (import.meta.env.DEV && !USE_API_IN_DEV) return id;
